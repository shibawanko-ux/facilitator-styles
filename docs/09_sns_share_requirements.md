...
| **awareness=design 告知** | 提供元・URL を明示する | 例：「ファシリスタイル診断 by awareness=design」「https://〜」 |
| **文字数** | SNSごとの制限を考慮する | X: 本文＋URLで余裕を持たせる、LINE: 長文は省略可 |

**投稿文テンプレート案（1行〜3行程度）**

```
ファシリスタイル診断の結果、私は「{タイプ名}」タイプでした！ {キャッチコピー（**除去）}
...
- ハッシュタグは「#ファシリスタイル」「#awarenessdesign」など、運用で決定。
...
| ハッシュタグを投稿文に含める | ✅ 可能 | テンプレートに「#ファシリスタイル」等を追加 |
...
1. **必須（すぐに反映可能）**
   ...
   - 投稿文に **ハッシュタグ**（例: #ファシリスタイル #awarenessdesign）を追加する（任意でON/OFFや文言は運用で調整）。
